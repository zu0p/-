<template>
  <v-card elevation="5" class="diary-card" :height="h" :width="w" @click="clickDiary">
    <v-container fluid class="diary-wrapper" :style="`background-image: url(${curDiary.diaryImage}); background-size : 250px`">
      <v-row class="card-contents">
        <v-col cols="1" sm="1" md="1" lg="1" id="diary-band" class="diary"> </v-col>
...
export default {
  ...
  methods: {
    ...
    clickDiary() {
      ...
      this.requestPageList(this.curDiary.id).then((res) => {
        // console.log(res)

...
const BASE_URL = "http://15.164.104.218/api/v1/diary";
...
const diaryStore = {
  ...
  mutations: {
    ...
    ADD_DIARY(state, diary) {
      if (state.store.diaryList.length >= 5) diary.hide = "box--hide";
      state.store.diaryList.push(diary);
      // console.log(state.store.diaryList)
    },
    UPDATE_DIARY(state, diary) {
      const res = state.store.diaryList.map((item) => {
        if (item.id == diary.diaryId) item.diaryName = diary.modifyName;
      });
    },
  ...
  actions: {
    ...
    createDiary({ commit }, diary) {
      return instanceWithAuth.post(`${BASE_URL}/create`, diary, { headers: { "Content-Type": "multipart/form-data" } });
    },
